...
const AUTH_APP_BASE_URL = "https://0512-auth-app.vercel.app";
const AUTH_APP_LOGIN_URL = `${AUTH_APP_BASE_URL}`;
const AUTH_APP_REGISTER_URL = `${AUTH_APP_BASE_URL}/register`;

console.log("DEBUG: AUTH_APP_LOGIN_URL =", AUTH_APP_LOGIN_URL);
console.log("DEBUG: AUTH_APP_REGISTER_URL =", AUTH_APP_REGISTER_URL);
...
export default function Home() {
  ...
  const fetchMusic = async (genre: string) => { /* ...fetch 邏輯... */ };
  const handleGenreSelect = (genre: string) => { /* ...選擇邏輯... */ };
  ...
  return (
    ...
    {/* *** 文字和按鈕 *** */}
            <div className="flex flex-col items-end"> {/* 外層包裹，讓文字和按鈕垂直排列並靠右 */}
              <p className="text-base text-muted-foreground hidden md:block mb-0 transform -translate-y-px"> {/* "發現..." 文字 */}
                發現、收藏並享受各種音樂
              </p>
              <div className="flex items-center gap-2"> {/* 按鈕組，作為 p 標籤的兄弟元素 */}
                <Link href={AUTH_APP_LOGIN_URL} legacyBehavior>
                  <a
                    target="_blank"
                    rel="noopener noreferrer"
                    className="px-6 py-1.5 text-xs font-medium text-slate-700 hover:bg-slate-100 rounded-md transition-colors"
                  >
                    登入
                  </a>
                </Link>
               <Link href={AUTH_APP_REGISTER_URL}  legacyBehavior>
                  <a
                    target="_blank"
                    rel="noopener noreferrer"
                    className="px-6 py-1.5 text-xs font-medium bg-primary text-primary-foreground hover:bg-primary/90 rounded-md transition-colors"
                  >
                    註冊
                  </a>
                </Link>
        ...
        <main className="container py-8 md:py-12">
          <MotionScroll>
            <h1 className="text-3xl font-bold tracking-tight sm:text-4xl md:text-4xl text-center mb-8">
              YouTube{" "}
